from agent.query_tools.forecast_loader import extract_from_xlsx
...
def calculate_capacity(df):
    if (df['Capacity_KT'] * 0.80 <= df['Predicted_Inventory_KT']).any(): 
        return df[df['Capacity_KT'] * 0.80 <= df['Predicted_Inventory_KT']]
    else:
        return ("All warehouses have sufficient capacity.")
